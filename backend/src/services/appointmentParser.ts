...
function parseGermanDate(raw: string, lineNumber: number): { date: Date; hasAsterisk: boolean } {
  const hasAsterisk = raw.trim().endsWith("*");
  const cleaned = raw.replace(/\*$/, "").trim();
  const match = cleaned.match(/^[^,]+,\s*(\d{1,2})\.\s*([A-Za-zäÄöÖüÜß\.]+)\s+(\d{4})$/);

  if (!match) {
    throw new HttpError(400, `Ungültiges Datumsformat in Zeile ${lineNumber}.`);
  }
  ...
  if (!month) {
    throw new HttpError(400, `Unbekannter Monat '${monthToken}' in Zeile ${lineNumber}.`);
  }
...
function parseTime(raw: string, lineNumber: number): Date {
  const match = raw.trim().match(/^(\d{2}):(\d{2})$/);
  if (!match) {
    throw new HttpError(400, `Ungültige Uhrzeit in Zeile ${lineNumber}.`);
  }
...
function extractFields(row: TokenizedRow): { dateRaw: string; fromRaw: string; toRaw: string; roomRaw: string } {
  ...
  if (timeIndexes.length < 2) {
    throw new HttpError(400, `Ungueltige Zeile ${row.lineNumber}: Von/Bis Zeit fehlt.`);
  }
  ...
  if (toIndex <= fromIndex) {
    throw new HttpError(400, `Ungueltige Zeile ${row.lineNumber}: Von/Bis Zeitreihenfolge ungueltig.`);
  }
  ...
  if (!dateRaw) {
    throw new HttpError(400, `Ungueltige Zeile ${row.lineNumber}: Datum fehlt.`);
  }

  if (!roomRaw) {
    throw new HttpError(400, `Ungueltige Zeile ${row.lineNumber}: Raum fehlt.`);
  }

  return { dateRaw, fromRaw, toRaw, roomRaw };
}

export function parseAppointments(rawText: string): Array<Omit<ParsedAppointment, "hasAsterisk"> & { type: AppointmentType }> {
  const lines = rawText
    .split(/\r?\n/)
