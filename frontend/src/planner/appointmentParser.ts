import { AppointmentType, PlannerAppointment, SnapshotAppointment } from "../api/types";
...
export function parseAppointments(rawText: string): SnapshotAppointment[] {
  const lines = rawText
    .split(/\r?\n/)
  ...
  return parsed.map((appointment) => ({
    ...
    type: hasAnyAsterisk
      ? appointment.hasAsterisk
        ? "Vorlesung"
        : "Uebung"
      : ("Vorlesung" satisfies AppointmentType)
  }));
}
...
export function formatAppointmentsForTextarea(appointments: PlannerAppointment[]): string {
  ...
  const rows = appointments.map((appointment, index) => {
    ...
    return `${index + 1}\t${dayAbbr}, ${date.getUTCDate()}. ${monthAbbr} ${date.getUTCFullYear()}${maybeStar}\t${appointment.timeFrom}\t${appointment.timeTo}\t${appointment.room}\t`;
  });
